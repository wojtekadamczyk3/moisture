...
# Methodology behind the mudNN

This document describes the methodology behind the mudNN, and the data used to train it. 

## About:

MudNN is a prioprietary model being developed by MUD which predicts the changes in the moisture content of the soil. The key aspect of the model is that it predicts the changes in the moisture, instead of the absolute moisture content. This allows us to do two things:
- backcast the rainfall, of which main indicator is the moisture change
- train the model on the synthetic data, which allows us to adress the problem of lack of data in the Global South

Traditionally the machine learning models are trained on the ground truth data. 


Initially the mudNN is trained on the ground truth data from the ISMN network. This data is only available for the Global North. This poses a problem, as the Global South is the region, which is most affected by lack of reliable rainfall data.

To adress this problem, mudNN will be trained on the synthetic data generated by synthetically imprinting a moisture signature of the moisture content on the satellite imagery. This allows to train the model on the various conditions and climates including the Global South.

## Input and output of the model

The model is a neural network, which takes as an input the GOES16/METEOSAT, Weather Backcasts and SMAP and outputs the changes in the moisture content of the soil. Specific architecture of the model still needs to be decided. 

<img src="assets/methodology/nn_diagram.png" alt="drawing" width="100%"/>




<img src="assets/methodology/v1_training_data.png" alt="drawing" width="40%"/>
